import { Layout, NotFound, PDF, Term } from "../../../components";
...
const OrderSuccessPage = ({ orderDetail }) => {
  ...
  useEffect(() => {
    ...
    if (orderDetail.role === "account") checkUser();
        ...
        <div className="flex space-x-6">
          <div className="w-[60%]">
            <div className="flex items-center my-6 space-x-2">
              <div>
                <CheckCircleIcon width={100} height={100} color="#8ec343" />
              </div>
              <div className="space-y-4">
                ...
                <div className="text-sm text-[#595959]">
                  <h2 className="block ">
                    We hope our product will meet your expectations. Let us know
                    if you have any questions.
                  </h2>
                  <h2 className="block ">
                    Hope we can see you again in next buy at Memoryzone
                  </h2>
                </div>
            ...
            <div className="grid ml-16 mt-10 grid-cols-2 grid-rows-2 gap-y-6 gap-x-8">
              ...
              <div className="flex flex-col text-sm space-y-4 text-[#595959]">
                <span className="text-xl text-[#000000]">Delivery address</span>
                <span>
                  {orderDetail.user
                    ? `${orderDetail.user.firstName} ${orderDetail.user.lastName}`
                    : orderDetail.guestName}
                ...
                <span>
                  {`${orderDetail.shippingAddress.ward.split("|")[1]}, ${
                    orderDetail.shippingAddress.district.split("|")[1]
                  }, ${orderDetail.shippingAddress.province.split("|")[1]}`}
                </span>
                <span>{orderDetail.shippingAddress.phoneNumber}</span>
              </div>
              <div className="flex flex-col text-sm space-y-4 text-[#595959]">
                <span className="text-xl text-[#000000]">Payment methods</span>
                <span>{orderDetail.paymentMethod}</span>
              </div>
              <div className="flex flex-col text-sm space-y-4 text-[#595959]">
                <span className="text-xl text-[#000000]">Shipping method</span>
          ...
          <div className="flex-1">
            <div className="border border-[#e1e1e1] bg-[#fafafa] divide-y divide-[#e1e1e1]">
              <span className="px-4  text-[#000000] text-sm font-semibold block my-2">
                Order [{orderDetail._id}] ({orderDetail.orderList.length})
              </span>
                        ...
                        <td>
                          <div className="relative">
                            <div className=" w-[50px] h-[50px] relative overflow-hidden rounded-md border border-[#e5e5e5]">
                              <Image
                                src={urlFor(
                                  orderDetail.productImage[index].image.image
                                ).url()}
                                layout="fill"
                                quality={100}
              ...
              <div className="flex items-center justify-between p-4">
                <span className="text-[#000000] text-[16px]">Total</span>
            ...
            trigger={() => {
              return (
                <div className="text-primary hover:text-[#006533] flex items-center space-x-2 cursor-pointer">
                  <div>
                    <PrinterIcon color="" width={35} height={35} />
                  </div>
